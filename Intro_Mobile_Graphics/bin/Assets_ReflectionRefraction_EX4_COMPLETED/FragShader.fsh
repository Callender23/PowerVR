...
void main()
{
	...
	highp vec4 diffuseColour    = texture2D(sTexture, vTexCoord);			// NEW
	highp vec4 reflectionColour = textureCube(sCubeMap, vReflectDir);		// NEW
	highp vec4 refractionColour = textureCube(sCubeMap, vRefractDir);		// NEW

    gl_FragColor = ((diffuseColour + reflectionColour + refractionColour) * diffuseLight) + specularLight;   // Changed
}
