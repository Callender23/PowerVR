...
const char* TriangleAttribNames[] = { "aVertex", "aTexCoord", "aNormal" };
...
const char* TriangleUniformNames[] = { "uMVPMatrix", "uMVMatrix", "uMVITMatrix", "uLightPosition", "uLightColour" };
...
bool Triangle::Init(pvr::Shell* shell, pvr::EglContext& context, const char* textureFilename, const char* textureCubemapFilename)
{
	...
	// Load the cubemap texture using PVR Utils
	_cubemapTexture = pvr::utils::textureUpload(*shell, textureCubemapFilename, context->getApiVersion() == pvr::Api::OpenGLES2);
	gl::BindTexture(GL_TEXTURE_CUBE_MAP, _cubemapTexture);
	...
	for (int i = 0; i < eNumUniforms; ++i) { _triangleShaderProgram.auiLoc[i] = gl::GetUniformLocation(_triangleShaderProgram.uiId, TriangleUniformNames[i]); }
...
void Triangle::Render(const glm::mat4& view, const glm::mat4& projection, const glm::vec3& lightPosition, const glm::vec4& lightColour)
{
	...
	gl::UniformMatrix4fv(_triangleShaderProgram.auiLoc[eMVMatrix], 1, GL_FALSE, glm::value_ptr(modelView));
	...
	// Create the light position based on the View matrix
	glm::vec3 viewLightPosition = glm::normalize(glm::vec3((view * glm::vec4(lightPosition, 0))));
	gl::Uniform3fv(_triangleShaderProgram.auiLoc[eLightPosition], 1, glm::value_ptr(viewLightPosition));
	// Send the directional light colour to the shader
	gl::Uniform4fv(_triangleShaderProgram.auiLoc[eLightColour], 1, glm::value_ptr(lightColour));

	// Binds the loaded textures
	gl::ActiveTexture(GL_TEXTURE0);
	gl::BindTexture(GL_TEXTURE_2D, _texture);
	// Set the sampler uniforms to corresponding texture units
	gl::Uniform1i(gl::GetUniformLocation(_triangleShaderProgram.uiId, "sTexture"), 0);

	gl::ActiveTexture(GL_TEXTURE1);
	gl::BindTexture(GL_TEXTURE_CUBE_MAP, _cubemapTexture);
	// Set the sampler uniforms to corresponding texture units
	gl::Uniform1i(gl::GetUniformLocation(_triangleShaderProgram.uiId, "sCubeMap"), 1);
...
void Triangle::SetPosition(float x, float y, float z) { _position = glm::translate(glm::vec3(x, y, z)); }
