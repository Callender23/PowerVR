...
Square::Square(void) : _vbo(0), _vertexStride(0), _texture(0), _x(0), _y(0), _z(0) {}
...
bool Square::Init(pvr::Shell* shell, pvr::EglContext& context, const char* textureFilename)
{
	...
	GLfloat afVertices[] = { // Vertex 1
							-0.4f+_x, -0.4f+_y, -2.0f+_z,	// Position 1
							 ...
							 // Vertex 3
							 0.4f+_x,  0.4f+_y, -2.0f+_z,	// Position 3
							 1.0f, 1.0f,					// Texture coodinate 3
							 // Vertex 4
							 -0.4f+_x, 0.4f+_y, -2.0f+_z,	// Position 4
	...
	_squareShaderProgram.uiId = pvr::utils::createShaderProgram(*shell, VertexShaderFile, FragmentShaderFile, attribs, attribIndices, 2, 0, 0);
...
void Square::Render(glm::mat4 mVP)
{
	...
	gl::UniformMatrix4fv(_squareShaderProgram.auiLoc[eMVPMatrix], 1, GL_FALSE, glm::value_ptr(mVP));
